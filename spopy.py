...
DEFAULT_CLIENT_ID = "77e240d32097423da4d574b772c721a4"
DEFAULT_REDIRECT_URI_PKCE = "https://spopy.amitkot.com/callback"
DEFAULT_REDIRECT_URI_LOCAL = "http://127.0.0.1:8888/callback"
DEFAULT_MARKET = ""
DEFAULT_TIMEOUT = 15
DEFAULT_RETRIES = 3
...
class CLIState:
    ...
    def uses_pkce(self) -> bool:
        """PKCE mode when no client secret is provided."""
...
def _env(primary: str, *fallbacks: str, default: str = "") -> str:
    """Read an env var with fallback names. First match wins."""
...
def _load_env() -> None:
    ...
    SPOTIFY_* vars are primary. SPOTIPY_* accepted as fallbacks for the
    spotipy library's native env vars.
    """
    _state.client_id = _env("SPOTIFY_CLIENT_ID", "SPOTIPY_CLIENT_ID", default=DEFAULT_CLIENT_ID)
    _state.client_secret = _env("SPOTIFY_CLIENT_SECRET", "SPOTIPY_CLIENT_SECRET")
    # Default redirect URI depends on whether we're in PKCE or OAuth mode
    default_redirect = DEFAULT_REDIRECT_URI_LOCAL if _state.client_secret else DEFAULT_REDIRECT_URI_PKCE
    _state.redirect_uri = _env("SPOTIFY_REDIRECT_URI", "SPOTIPY_REDIRECT_URI", default=default_redirect)
    _state.cache_path = _env("SPOTIFY_CACHE_PATH", "SPOTIPY_CACHE_PATH", default=DEFAULT_CACHE_PATH)
    _state.username = _env("SPOTIFY_USERNAME", "SPOTIPY_USERNAME")
    _state.scopes = _env("SPOTIFY_SCOPES", default=DEFAULT_SCOPES)
    _state.default_device_id = _env("SPOTIFY_DEFAULT_DEVICE_ID")
    _state.default_device_name = _env("SPOTIFY_DEFAULT_DEVICE_NAME")
    _state.market = _env("SPOTIFY_MARKET")
    _state.output = _env("SPOTIFY_OUTPUT", default=OutputMode.RICH)
    _state.timeout = int(_env("SPOTIFY_TIMEOUT_SECONDS", default=str(DEFAULT_TIMEOUT)))
    _state.retries = int(_env("SPOTIFY_RETRIES", default=str(DEFAULT_RETRIES)))
    _state.backoff_factor = float(_env("SPOTIFY_BACKOFF_FACTOR", default=str(DEFAULT_BACKOFF_FACTOR)))
    _state.debug = _env("SPOTIFY_DEBUG", default="0") == "1"
    _state.open_browser = _env("SPOTIFY_OPEN_BROWSER", default="1") == "1"
    _state.no_color = _env("SPOTIFY_NO_COLOR", default="0") == "1"
    _state.token_import_json = _env("SPOTIFY_TOKEN_IMPORT_JSON")
    _state.state_file = _env("SPOTIFY_STATE_FILE")
...
def _build_auth_manager(open_browser: bool = False) -> SpotifyOAuth | SpotifyPKCE:
    ...
    if not _state.auth_configured():
        ...
        if not _state.uses_pkce and not _state.client_secret:
    ...
    if _state.uses_pkce:
        logger.debug("Using PKCE auth flow")
        return SpotifyPKCE(
    ...
    logger.debug("Using OAuth auth flow (client secret provided)")
    return SpotifyOAuth(
        client_id=_state.client_id,
...
def _pkce_state_path() -> Path:
    """Path for persisting PKCE code_verifier between auth url and callback."""
    return Path(_state.cache_path).with_suffix(".pkce")


def _save_pkce_state(auth_manager: SpotifyOAuth | SpotifyPKCE, state: str) -> None:
    """Save PKCE code_verifier and state so callback-url can use them."""
    data: dict[str, str] = {"state": state}
    if isinstance(auth_manager, SpotifyPKCE) and hasattr(auth_manager, "_code_verifier"):
        data["code_verifier"] = auth_manager._code_verifier
    p = _pkce_state_path()
    p.parent.mkdir(parents=True, exist_ok=True)
...
def _restore_pkce_state(auth_manager: SpotifyOAuth | SpotifyPKCE) -> str | None:
    """Restore PKCE code_verifier into the auth manager. Returns saved state."""
    p = _pkce_state_path()
    if not p.exists():
        return None
    try:
        data = json.loads(p.read_text())
        if isinstance(auth_manager, SpotifyPKCE) and "code_verifier" in data:
            auth_manager._code_verifier = data["code_verifier"]
        return data.get("state")
    except Exception:
        return None


def _cleanup_pkce_state() -> None:
    """Remove the PKCE state file after successful auth."""
    p = _pkce_state_path()
    if p.exists():
...
def _require_device(sp: spotipy.Spotify) -> str:
    ...
    if not dev:
        _die(
            ...
            ExitCode.API_ERROR,
            hint="Open Spotify on a device, or set SPOTIFY_DEFAULT_DEVICE_ID / SPOTIFY_DEFAULT_DEVICE_NAME.",
        )
    return dev
...
app.add_typer(auth_app, rich_help_panel="Setup & Diagnostics")
app.add_typer(devices_app, rich_help_panel="Setup & Diagnostics")
app.add_typer(track_app, rich_help_panel="Browse & Search")
app.add_typer(album_app, rich_help_panel="Browse & Search")
app.add_typer(artist_app, rich_help_panel="Browse & Search")
app.add_typer(playlist_app, rich_help_panel="Browse & Search")
app.add_typer(library_app, rich_help_panel="Browse & Search")
app.add_typer(queue_app, rich_help_panel="Playback")
app.add_typer(top_app, rich_help_panel="Discovery")
app.add_typer(genre_app, rich_help_panel="Discovery")
app.add_typer(mood_app, rich_help_panel="Discovery")
...
def _global_options(
    ...
    """spopy — production-quality Spotify control from your terminal."""
    _load_env()
    # Auto-detect non-interactive (piped) usage: default to JSON + yes
    if not sys.stdin.isatty():
        if not json_out and not plain:
            json_out = True
        yes = True
    _apply_output_flags(json_out, plain, debug, no_color)
...
def auth_setup_guide() -> None:
    """Print step-by-step instructions for setting up spopy."""
    guide = """\
[bold]spopy — Setup Guide[/]

[bold]Quick Start (zero config)[/]

  spopy includes a built-in Spotify app. Just run:

  [green]spopy auth login[/]

  Your browser will open, you'll approve access, and you're done.
  For remote/headless servers, use:

  [green]spopy auth url[/]       → open the URL in a browser elsewhere
                        → after approving, copy the command from the page
  [green]spopy auth callback-url '<paste>'[/]

[bold]Advanced: Use your own Spotify app[/]

  If you want your own app (e.g. for rate limit isolation):

  1. Go to [link=https://developer.spotify.com/dashboard]https://developer.spotify.com/dashboard[/link]
  2. Create an app with redirect URI: [green]http://127.0.0.1:8888/callback[/]
  3. Set environment variables:

     [green]export SPOTIFY_CLIENT_ID='your_client_id'[/]
     [green]export SPOTIFY_CLIENT_SECRET='your_client_secret'[/]
     [green]export SPOTIFY_REDIRECT_URI='http://127.0.0.1:8888/callback'[/]

  Setting SPOTIFY_CLIENT_SECRET switches spopy to the classic OAuth flow.

[bold]Transfer token to a remote server[/]

  On local:   [green]spopy auth login[/]
  On local:   [green]spopy auth export-token-info --raw --yes > token.json[/]
  On remote:  [green]spopy auth import-token-info token.json[/]
...
def auth_status() -> None:
    ...
    def _plain() -> None:
        print(f"auth_flow\t{'pkce' if _state.uses_pkce else 'oauth'}")
        print(f"client_id\t{_mask_token(_state.client_id, 4) if _state.client_id else 'NOT SET'}")
        print(f"client_secret\t{'not needed' if _state.uses_pkce else ('set' if _state.client_secret else 'NOT SET')}")
    ...
    def _data() -> dict[str, Any]:
        d: dict[str, Any] = {
            "auth_flow": "pkce" if _state.uses_pkce else "oauth",
...
def auth_url(
    ...
    _save_pkce_state(oauth, state_val)
    ...
    if json_out or _state.flag_json:
        ...
        _out.print("  2. Approve the app (first time only).")
        if _state.uses_pkce:
            _out.print("  3. You'll be redirected to a page with a ready-to-paste command.")
            _out.print("  4. Copy the command and run it in your terminal.")
        else:
            _out.print('  3. Your browser will show an [yellow]"Unable to connect"[/] error.')
            _out.print("     [bold]This is expected.[/] Copy the [bold]entire URL[/] from the address bar.")
            _out.print("  4. Run: [green]spopy auth callback-url '<paste_the_full_url>'[/]")
...
def auth_login(
    ...
    if not should_open:
        ...
        if _state.uses_pkce:
            ...
            _out.print("After approving, you'll see a page with a ready-to-paste command.")
            _out.print("Copy it and run it here.")
        else:
            _out.print("\n[bold]No browser mode.[/] Open the URL above in a browser.")
            _out.print('After approving, your browser will show an [yellow]"Unable to connect"[/] error.')
            _out.print("[bold]This is expected.[/] Copy the URL from the address bar, then run:")
            _out.print("  [green]spopy auth callback-url '<paste_the_full_url>'[/]")
        return

    if _state.uses_pkce:
        _out.print("\nAfter approving, you'll see a page with a command to copy and paste below.")
    else:
    ...
    redirect_response = typer.prompt("Paste here")
    # Strip command prefix if user pasted the full command from the callback page
    redirect_response = re.sub(r"^spopy\s+auth\s+callback-url\s+['\"]?", "", redirect_response.strip())
    redirect_response = redirect_response.rstrip("'\"")
    try:
        ...
        if isinstance(token_info, str):
            # SpotifyPKCE may return just the token string; fetch full info from cache
            token_info = oauth.get_cached_token()
            if not token_info:
                _die("Auth succeeded but could not read cached token.", ExitCode.INTERNAL_ERROR)
...
def auth_callback_url(
    url: str = typer.Argument(..., help="Full redirected callback URL or paste from callback page."),
) -> None:
    """Exchange a callback URL (pasted from the browser) for tokens."""
    # Strip command prefix if user pasted the full command from callback page
    url = re.sub(r"^spopy\s+auth\s+callback-url\s+['\"]?", "", url.strip())
    url = url.rstrip("'\"")

    oauth = _build_auth_manager(open_browser=False)
    # Restore PKCE code_verifier from the auth url / login step
    _restore_pkce_state(oauth)
...
@app.command("current", rich_help_panel="Now Playing")
...
@app.command("pause", rich_help_panel="Playback")
def pause_cmd() -> None:
    """Pause playback."""
    sp = _get_spotify()
    device = _select_device(sp)
...
@app.command("resume", rich_help_panel="Playback")
def resume_cmd() -> None:
...
@app.command("stop", rich_help_panel="Playback")
def stop_cmd() -> None:
...
@app.command("next", rich_help_panel="Playback")
def next_cmd() -> None:
    """Skip to next track."""
    sp = _get_spotify()
    device = _select_device(sp)
...
@app.command("previous", rich_help_panel="Playback")
...
@app.command("seek", rich_help_panel="Playback")
def seek_cmd(
...
@app.command("volume", rich_help_panel="Playback")
def volume_cmd(
...
@app.command("repeat", rich_help_panel="Playback")
def repeat_cmd(
...
@app.command("shuffle", rich_help_panel="Playback")
def shuffle_cmd(
...
@app.command("discover", rich_help_panel="Discovery")
